...
study_xgb = optuna.create_study(
    ...
    sampler=optuna.samplers.TPESampler(seed=RANDOM_STATE),
    storage=storage,
    study_name="gaussian_xgb",
    load_if_exists=True
)
print(f"Study: gaussian_xgb | Completed trials: {len(study_xgb.trials)}")
study_xgb.optimize(objective_xgb, n_trials=N_TRIALS, show_progress_bar=True)

print(f"\nBest XGB CV accuracy: {study_xgb.best_value:.4f}")
...
if study_xgb.best_value > best_overall_score:
    ...
    best_overall_model = "optimized_xgb"
...
study_lgb = optuna.create_study(
    ...
    sampler=optuna.samplers.TPESampler(seed=RANDOM_STATE),
    storage=storage,
    study_name="gaussian_lgb",
    load_if_exists=True
)
print(f"Study: gaussian_lgb | Completed trials: {len(study_lgb.trials)}")
study_lgb.optimize(objective_lgb, n_trials=N_TRIALS, show_progress_bar=True)

print(f"\nBest LGB CV accuracy: {study_lgb.best_value:.4f}")
...
if study_lgb.best_value > best_overall_score:
    ...
    best_overall_model = "optimized_lgb"
...
if best_overall_model == "baseline_rf":
...
elif best_overall_model == "optimized_xgb":
    from sklearn.preprocessing import LabelEncoder
    le = LabelEncoder()
    y_encoded = le.fit_transform(y)
    final_model = xgb.XGBClassifier(**best_overall_params)
    ...
    print("\nDone! Best CV accuracy:", best_overall_score)
    exit()

elif best_overall_model == "optimized_lgb":
    from sklearn.preprocessing import LabelEncoder
    le = LabelEncoder()
    y_encoded = le.fit_transform(y)
    final_model = lgb.LGBMClassifier(**best_overall_params)
    final_model.fit(X, y_encoded)
